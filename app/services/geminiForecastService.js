 ...
 */
function buildForecastPrompt(history, horizon = 7) {
...
- Today's total bookings: ${todayQty} cylinders
- Yesterday's total bookings: ${yesterdayQty} cylinders
...
1. **PRIORITIZE RECENT PATTERNS**: If agent had ${todayQty} cylinders booked today and ${yesterdayQty} yesterday, they will likely need similar quantity tomorrow (around ${Math.max(todayQty, yesterdayQty, Math.round(last3DaysAvg))} cylinders)
2. Recent booking patterns (last 2-3 days) are more important than long-term averages
3. If today's bookings are ${todayQty} and yesterday's are ${yesterdayQty}, tomorrow should be similar (around ${Math.max(todayQty, yesterdayQty)} cylinders)
...
async function generateForecast(agentId, horizon = 7, historyDays = 60) {
  ...
  try {
    ...
    // Step 2: Build prompt
    const prompt = buildForecastPrompt(history, horizon);
    ...
    forecast = forecast.map((entry, index) => {
      ...
      if (index === 0 && (todayQty > 0 || yesterdayQty > 0)) {
        // If agent had bookings today/yesterday, tomorrow should be similar
    ...
    console.log(`Successfully generated forecast for agent ${agentId}: ${forecast.length} days`);
    return forecast;
  } catch (error) {
...
function generateDefaultForecast(horizon, recentHistory = []) {
  ...
  // Use recent average if available, otherwise default to 5
  const baseDemand = Math.max(Math.round(last3DaysAvg), Math.round((todayQty + yesterdayQty) / 2), 5);
